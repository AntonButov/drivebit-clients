...
class WebDataService {
    ...
    fun getCars(): List<CarData> {
        return listOf(
            ...
            CarData("Hyundai Creta", "📍 Санкт-Петербург", "3500₽ / сутки", "2020 год", "Автоматическая", "Бензин", "2.0 л")
        )
    }
    
    fun getFeatures(): List<FeatureData> {
        return listOf(
            FeatureData("₽", "Дешевле проката на 40%", "Работаем без посредников и лишних наценок. Аренда напрямую у владельцев."),
            FeatureData("🛡️", "Полная страховка", "Решаем проблемы за вас. Ваша ответственность — 0₽. Все риски на нас."),
            FeatureData("📞", "Поддержка 24/7", "Поможем в любой ситуации в дороге. На связи круглосуточно."),
            FeatureData("⏱️", "Честный залог", "Возвращаем сразу после сдачи авто. Никаких задержек и скрытых удержаний.")
        )
    }
}
