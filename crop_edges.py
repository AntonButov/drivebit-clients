#!/usr/bin/env python3
"""
Универсальный скрипт для обрезания изображений по краям
Убирает указанный процент с каждой стороны (слева, справа, сверху, снизу)
"""
...
def crop_images_edges(directory, crop_percent=20, quality=85):
    """
    Обрезает изображения в указанной директории по краям
    
    Args:
        directory: путь к папке с изображениями
        crop_percent: процент для обрезания с каждой стороны (по умолчанию 20%)
    ...
    print(f"Обрезаем {crop_percent}% с каждой стороны, качество: {quality}")
    print("-" * 60)
    ...
    for image_path in image_files:
        try:
            ...
            with Image.open(image_path) as img:
                ...
                new_width = int(original_width * (1 - 2 * crop_ratio))
                new_height = int(original_height * (1 - 2 * crop_ratio))
                
                # Вычисляем отступы
                left_crop = int(original_width * crop_ratio)
                top_crop = int(original_height * crop_ratio)
                right_crop = original_width - new_width - left_crop
...
def main():
    """Главная функция"""
    if len(sys.argv) < 2:
        print("Использование: python3 crop_edges.py <путь_к_папке> [процент_обрезания] [качество]")
        print("Пример: python3 crop_edges.py images/searchbackground 20 85")
        print("Обрезает указанный процент с каждой стороны (слева, справа, сверху, снизу)")
        return
    
    directory = sys.argv[1]
    crop_percent = int(sys.argv[2]) if len(sys.argv) > 2 else 20
    quality = int(sys.argv[3]) if len(sys.argv) > 3 else 85
    
    crop_images_edges(directory, crop_percent, quality)
